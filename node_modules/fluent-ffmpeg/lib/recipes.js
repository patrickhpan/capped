  ...
  proto.screenshots = function(config, folder) {
    ...
    async.waterfall([
      ...
      function fixPattern(next) {
        ...
        if (config.timemarks.length > 1 && !pattern.match(/%(0*(\.0*)?s|0*i)/)) {
      ...
      function replaceVariableTokens(pattern, next) {
        var filenames = config.timemarks.map(function(t, i) {
          return pattern
            ...
            .replace(/%(0*)(\.0*)?s/g, function (match, leftPadding, round) {
              var timestamp = utils.timemarkToSeconds(t)
              if (round !== undefined) {
                timestamp = timestamp.toFixed(round.length - 1);
              }
              timestamp = '' + timestamp;
              let timestampBeforeDecimal = timestamp.indexOf('.') === -1 ? timestamp.length : timestamp.indexOf('.')
              return leftPadding.substr(0, Math.max(0, leftPadding.length - timestampBeforeDecimal)) + timestamp;
            })
